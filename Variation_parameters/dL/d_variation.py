...
lab = ["$d_\mathrm{L} = 1 ~ \mathrm{Gpc}$", "$d_\mathrm{L} = 2 ~ \mathrm{Gpc}$", "$d_\mathrm{L} = 3 ~ \mathrm{Gpc}$"]

for i in range(0, 3):
    h = gen_wave(M, mu, a, p0, e0, x0, 1 + i, qS, phiS, qK, phiK, Phi_phi0, Phi_theta0, Phi_r0, T=T, dt=dt)
...
ax.grid()
ax.set_title(r"GW for different $\bf{d_\mathrm{L}}$", y = 1.02)

# plt.savefig("Variation_parameters/dL/d_vgl.png")
